...
function getProductsByCategory(products, category) {
   ...
   */
  const newArray = products.filter((product) => product.category === category);
...
function findProductById(products, id) {
   ...
   */
  const newArray = products.find((product) => product.id === id);
  return newArray;
}
...
function calculateTotalPrice(products) {
   ...
   */
  const newArray = products.reduce((acc, product) => acc + product.price, 0);
  return newArray;
}
